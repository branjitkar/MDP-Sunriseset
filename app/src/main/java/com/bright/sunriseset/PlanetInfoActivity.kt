...
class PlanetInfoActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnSwitchLanguage.setOnClickListener {
            onLanguageSwitchButtonPressed()
        }
    }

    //dynamically switch locale to chinese
    fun switchLanguage(context: Context, localeToSwitch: Locale) {
        val localeUpdatedContext = ContextUtils.updateLocale(context, localeToSwitch)
        recreate()
    }

    // Call this function when the language switch button is pressed
    fun onLanguageSwitchButtonPressed() {
        val localeToSwitch = Locale.CHINA
        switchLanguage(this, localeToSwitch) // Replace 'this' with the appropriate context
    }
    ...
    private suspend fun fetchTime(type: String): LocalDateTime? {
        return try {
            val apiUrl =
                URL("https://api.sunrise-sunset.org/json?lat=40.808960&lng=-74.136790&formatted=0")
